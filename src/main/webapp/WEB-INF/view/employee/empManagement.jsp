...
<script type="text/javascript">
	...
	$(document).ready(function() {
		let
		dateElement = document.getElementById('birth');
	...
	function fn_empManagementList(curpage) {
		...
		callAjax("/employee/listEmpManagement.do", "post", "text", true, param,
				EmpManagementlistcallback);
	}
	...
	function fn_EmpManagementlistcallback(returndata, curpage) {
		...
		// 페이지 네비게이션 생성		
		var paginationHtml = getPaginationHtml(curpage, totcnt, pageSize,
				pageBlock, 'fn_empManagementList');
	...
	function fn_retirefn(loginID) {
		...
		var userRetirecallback = function(resultdata) {
			...
			gfModalPop("#retireModal"); //퇴사자 모달 
		}

		callAjax("/employee/selectUserRetire.do", "post", "json", true, param,
				userRetirecallback);
	}
	...
	function fn_formRetireinit(data) {
		...
		$("#sal_btre").val(data.sal_bt); // 세전급여 
		$("#loginId").attr("readonly", true); //작성자 
		$("#name").attr("readonly", true); //사원명
		$("#hire_date").attr("readonly", true); //입사일 
		//최소 입사일 이후로 되도록 설젇
		...
		if (data.user_du == 'N') {
			...
			$("#retire_dateY").attr("readonly", true); //입사일 
			$("#retire_r").attr("readonly", true); //퇴사일 
			$("#btnRetireSave").hide();
		} else {
			//$("#retire_dateY").hide();
			//$("#retire_date").show();
	...
	function fn_userRetireSave() { //퇴사 처리
	...
	function fn_saveRetire() {
		var param = {
			...
			sal_btre : $("#sal_btre").val(), //세전급여 
			hire_date : $("#hire_date").val()
		}

		var saveUserRetirecallback = function(savereturn) {
			console.log("saveUserRetirecallback : "
					+ JSON.stringify(savereturn));
		...
		callAjax("/employee/saveUserRetire.do", "post", "json", true, param,
				saveUserRetirecallback);
	}
	...
	function fn_empManagementInfoSelect(loginID) {
		...
		var selectEmpManagementcallback = function(selectresult) {
			console.log("selectEmpManagementcallback"
 	...
 	function fn_empManagementInfoModal(loginID) {
		if (loginID == null || loginID == "") {//신규 사원등록 
			//로그인 아이디 가지고 오기
			fn_empManagementID();
 	...
 	function fn_empManagementID() {
		...
		var selectempManagementIDcallback = function(savereturn) {
			console.log("selectempManagementIDcallback : "
					+ JSON.stringify(savereturn));
	...
	function fn_empManagementforminit(empManagementModal) {
		if (empManagementModal == "" || empManagementModal == null
				|| empManagementModal == undefined) { // 신규
			...
			$("#dept_name").val(empManagementModal.dept_cd).prop("selected",
					true);
			...
			$("#upfile").attr("disabled", true);
 			...
 			if (empManagementModal.att_ori == ""
					|| empManagementModal.att_ori == null
					|| empManagementModal.att_ori == undefined) {
				$("#fileinfo").empty();
			} else {
				var filearr = empManagementModal.att_ori.split(".");
				var previewhtml = "";
				if (filearr[1] == "jpg" || filearr[1] == "png"
						|| filearr[1] == "jpeg" || filearr[1] == "gif") {
		...
		function fn_empManagementfileSave() {
			...
			var saveEmpManagementcallback = function(savereturn) {
				console.log("saveEmpManagementcallback : "
						+ JSON.stringify(savereturn));
			...
			callAjaxFileUploadSetFormData("/employee/saveEmpManagementfile.do",
					"post", "json", true, dataWithFile,
					saveEmpManagementcallback);
		...
		function fValidateEmpDv() {
			...
			var chk = checkNotEmpty([ [ "upfile", "파일을 첨부해주세요" ],
					[ "name", "이름을 입력해주세요" ], [ "Con1", "주민번호 앞자리를 입력해주세요" ],
					[ "Con2", "주민번호 뒷자리를 입력해주세요" ], [ "birth", "생일을 입력해주세요" ],
					[ "degree", "학력을 선택해주세요" ], [ "mail", "이메일을 입력해주세요" ],
					[ "tel2", "휴대전화 중간 번호를 입력해주세요" ],
					[ "tel3", "휴대전화 끝 번호를 입력해주세요" ],
					[ "zip_code", "우편번호를 입력해주세요" ], [ "bank", "은행을 선택해주세요" ],
					[ "account", "계좌를 입력해주세요" ], [ "addr", "주소를 입력해주세요 " ],
					[ "addr_detail", "상세주소를 입력해주세요" ],
					[ "dept_name", "부서를 선택해주세요" ],
					[ "user_type", "직책을 선택해주세요" ],
					[ "detail_name", "직급을 선택해주세요" ],
					[ "user_date", "입사일을 입력해주세요" ], [ "sal_ad", "연봉을 입력해주세요" ] ]);
		...
		function fValidateEmpDv22() {
			...
			var chk = checkNotEmpty([ [ "name", "이름을 입력해주세요" ],
...
<body>
	...
	<form id="empManagement" action="" method="">
		...
		<input type="hidden" name="action" id="action" value=""> <input
			type="hidden" name="retireYN" id="retireYN" value="">
						...
						<div class="content">
							...
							<p class="Location">
								<a href="#" class="btn_set home">메인으로</a> <a href="#"
									class="btn_nav bold">인사/급여</a> <span class="btn_nav bold">사원
									관리</span> <a href="#" class="btn_set refresh">새로고침</a>
							</p>

							<p class="conTitle">
								<span>사원 기본정보 </span> <span class="fr"> <c:set
										var="nullNum" value=""></c:set> <a class="btnType blue"
									href="javascript:fn_empManagementInfoModal(${nullNum});"
							...
							<table width="100%" cellpadding="5" cellspacing="0" border="1"
								...
								<tr style="border: 0px; border-color: blue">
									...
									<td width="10" height="25" style="font-size: 120%"><select
										id="selectDepart" name="selectDepart" style="width: 100px;">
									...
									<td width="10" height="25" style="font-size: 120%"><select
										id="sampleconcode1" name="sampleconcode1"
										style="width: 100px;">

									</select></td>
									<td width="15" height="25" style="font-size: 100%">&nbsp;</td>
									<td width="10" height="25" style="font-size: 120%"><select
										id="selectOP" name="selectOP" style="width: 100px;">
											<option value="" selected="selected">선택</option>
									...
									<td width="50" height="25" style="font-size: 100%"><input
										...
										id="searchinfo" name="searchinfo"></td>
									<td width="50" height="25" style="font-size: 100%">입사일</td>
									<td width="50" height="25" style="font-size: 100%"><input
										type="date" style="width: 120px; height: 30px;" id="from_date"
										name="from_date"></td>
									<td width="10" height="60" style="font-size: 100%"><input
										type="date" style="width: 120px; height: 30px;" id="to_date"
										name="to_date"></td>
							...
							<div style="margin-bottom: 10px;">
								<input type="button" class="btnTypeEMP" id="no_retire"
									name="retire" value="재직자"> <input type="button"
									class="btnTypeEMP" id="yes_retire" name="retire" value="퇴직자">
							</div>
		...
		<div id="empManage" class="layerPop layerType2" style="width: 1000px;">
			<input type="hidden" id="empManageNo" name="empManageNo" value="">
			<!-- 수정시 필요한 num 값을 넘김  -->
			...
			<dl>
				...
				<dd class="content">
					...
					<table class="row">
						...
						<tbody>
							<tr>
								<td rowspan="3"><input type="file" id="upfile"
									name="upfile" style="display: none;"
									accept="image/gif, image/jpeg, image/png" />
									<div id="fileinfo"
										style="width: 100px; height: 100px; margin: 0 auto;"
										title="클릭시 사진 첨부 됩니다."></div></td>
								<th scope="row">사번 <span class="font_red">*</span></th>
								<td><input type="text" class="inputTxt"
									style="width: 151px !important;" name="loginID" id="loginID"
									required="required" /></td>
								<th scope="row">성명<span class="font_red">*</span></th>
								<td scope="row" colspan="2"><input type="text"
									class="inputTxt" name="name" id="name" required="required" /></td>
							</tr>
							<tr>
								<th scope="row">주민번호 <span class="font_red">*</span></th>
								<td><input type="text" maxlength="6"
									style="width: 65px !important;" class="inputTxt" name="Con1"
									id="Con1"
									oninput="this.value = this.value.replace(/[^0-9]/g, '');"
									required="required" /> - <input type="text" maxlength="7"
									style="width: 69px !important;" class="inputTxt" name="Con2"
									id="Con2"
									oninput="this.value = this.value.replace(/[^0-9]/g, '');"
									required="required" /></td>
								<th scope="row">성별 <span class="font_red">*</span></th>
								<td colspan="2"><input type="radio" name="gender"
									id="genderM" value="1" checked="checked"> 남 <input
									type="radio" name="gender" id="genderF" value="2"> 여</td>
							</tr>
							<tr>
								<th scope="row">생년월일<span class="font_red">*</span></th>
								<td><input type="date" name="birth" id="birth"
									style="width: 150px; height: 27px;" required="required"></td>
								<th scope="row">최종학력<span class="font_red">*</span></th>
								<td colspan="2"><select id="degree" name="degree"
									style="width: 95px !important;" required="required">
								</select></td>
							</tr>
							<tr>
								<th scope="row">이메일 <span class="font_red">*</span></th>
								<td colspan="2"><input type="email" class="inputTxt p100"
									name="mail" id="mail" onchange="fn_emailcheck()"
									required="required" /></td>
								<th scope="row">휴대 전화 <span class="font_red">*</span></th>
								<td colspan="2"><input type="text" maxlength="3"
									style="width: 45px !important;" class="inputTxt" name="tel1"
									id="tel1" value="010" readonly="readonly" required="required" />
									- <input type="text" maxlength="4"
									style="width: 50px !important;" class="inputTxt" name="tel2"
									id="tel2"
									oninput="this.value = this.value.replace(/[^0-9]/g, '');"
									required="required" /> - <input type="text" maxlength="4"
									style="width: 50px !important;" class="inputTxt" name="tel3"
									id="tel3"
									oninput="this.value = this.value.replace(/[^0-9]/g, '');"
									required="required" /></td>
							</tr>
							<tr>
								...
								<td><input type="text" class="inputTxt p100"
									name="zip_code" id="zip_code" style="width: 108px !important;"
									required="required" /></td>

								<td><input type="button" value="우편번호 검색" id="zipFind"
									onclick="execDaumPostcode()"
									style="width: 110px; height: 24px;" /></td>
								<th scope="row">은행 계좌<span class="font_red">*</span></th>
								<td><select id="bank" name="bank"
									style="width: 95px !important;" required="required">
								</select></td>
								<td><input type="text" class="inputTxt p100" name="account"
									id="account"
									oninput="this.value = this.value.replace(/[^0-9]/g, '');"
							...
							<tr>
								...
								<td colspan="3"><input type="text" class="inputTxt p100"
									name="addr_detail" id="addr_detail" placeholder="상세 주소"
									required="required" /></td>
							</tr>
							<tr>
								<th scope="row">부서<span class="font_red">*</span></th>
								<td><select id="dept_name" name="dept_name"
									style="width: 143px !important;" required="required">
										<option value="" selected="selected">선택</option>
										<c:forEach items="${departmentList}" var="list">
											<option value="${list.dept_cd}">${list.dept_name}</option>
										</c:forEach>
								...
								<td><input type="text" class="inputTxt" name="dept_cd"
									id="dept_cd" readonly="readonly" required="required" /></td>
								<th scope="row">직책<span class="font_red">*</span></th>
								<td><select id="user_type" name="user_type"
									style="width: 143px;" required="required">

								</select></td>
							</tr>
							<tr>
								<th scope="row">직급<span class="font_red">*</span></th>
								<td><select id="detail_name" name="detail_name"
									style="width: 143px !important;" required="required">

								</select></td>
								<th scope="row">직급코드<span class="font_red">*</span></th>
								<td><input type="text" class="inputTxt" name="job_cd"
									id="job_cd" readonly="readonly" /></td>
								<th scope="row">재직구분 <span class="font_red">*</span></th>
								<td><input type="radio" name="user_du" id="work" value="Y"
									checked="checked"> 재직 <input type="radio"
									name="user_du" id="break" value="B"> 휴직 <input
									type="radio" name="user_du" id="retire" value="N"> 퇴직</td>
							</tr>
							<tr>
								<th scope="row">입사일</th>
								<td><input type="date" id="user_date" name="user_date"
									style="width: 140px; height: 27px;" required="required">
								</td>
								<th scope="row" id="retireth">퇴사일</th>
								<td id="retiretd"><input type="date" id="user_redate"
									name="user_redate" style="width: 140px; height: 27px;"
									readonly="readonly"></td>
								<th scope="row">근무연차</th>
								<td><input type="text" class="inputTxt" name="workTot"
									id="workTot" readonly="readonly" /></td>
							</tr>
							<tr>
								<th scope="row">연봉</th>
								<td><input type="text" class="inputTxt" name="sal_ad"
									id="sal_ad" onkeyup="inputNumberFormat(this);"
									style="width: 140px !important;" required="required" /></td>
								<th scope="row">퇴직금</th>
								<td><input type="text" class="inputTxt" name="retireMc"
									id="retireMc" style="width: 160px !important;"
									readonly="readonly" /></td>
							</tr>
						</tbody>
					</table>
					...
					<div class="btn_areaC mt30">
						<a href="" class="btnType blue" id="btnSaveEmpManagement"
							name="btn"><span>저장</span></a> <a href="" class="btnType blue"
							id="btnUpdateEmpManagement" name="btn" style="display: none"><span>수정</span></a>
						<a href="" class="btnType gray" id="btnCloseEmpManagement"
		...
		<!-- 퇴사처리 모달팝업 -->
		<div id="retireModal" class="layerPop layerType2"
			style="width: 600px;">
			<input type="hidden" id="loginIdre" name="loginIdre" value="">
			<!-- 수정시 필요한 num 값을 넘김  -->
					...
					<table class="row">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="inputTxt p100" name="namere"
									id="namere" /></td>
							</tr>
							<tr>
								<th scope="row">입사일</th>
								<td><input type="text" class="inputTxt p100"
									name="hire_date" id="hire_date" /></td>
								<th scope="row">퇴사일</th>
							<td >
							...
							<tr>
								<th scope="row" colspan="4" style="text-align: center;">퇴사
									사유</th>
							</tr>
							<tr>
								<td colspan="4"><textarea class="inputTxt p100"
										name="retire_r" id="retire_r" style="resize: none;">
								</textarea></td>
							</tr>
						</tbody>
					</table>
