...
<script type="text/javascript">
	...
	function fSaveModalEmpTaApply(){
		...
		const diffDate = end_date.getTime() - start_date.getTime();
		
		const vac_date =  Math.abs(diffDate / (1000 * 60 * 60 * 24) + 1); // 밀리세컨 * 초 * 분 * 시 = 일
			
    	if($("#dva_use").html() < vac_date){
    		alert('보유한 연차보다 기간이 깁니다.');
    		return
    	};
	...
	function fPopModalEmpTaApply(){
		...
		if( $("#dva_use").html()>0){
			...
			location.href = "/employee/empTaApply.do";
	 ...
	 function fPersonalInfoResult(data){
				...
				console.log('부서 : ',data.empTaLoginInfoModel.dept_name);
	...
	/* 반려사유 단건 조회 */
	 function fn_EmpTaApplyDtlModal(idx,state,loginID) {
		...
		callAjax("/employee/empDtlTaApply.do", "post", "json", true, param, resultCallback);
			...
			if($("#userType").val() == "A" && loginID == $("#loginIDa").val()) {
				
				console.log("userType : " + $("#userType").val());
				alert("자신의 연차사용은 승인 할 수 없습니다.");
				...
				if($("#userType").val() == "A") {
					$("#fvactype" ).prop('disabled', true); 
					$("#start_date").prop('readonly', true); 
					$("#end_date").prop('readonly', true);
					$("#vac_content").prop('readonly', true);
					$("#tel").prop('readonly', true);
				}
					
				if($("#userType").val() != "A")	{
					$("#fvactype" ).prop('disabled', false); 
					$("#start_date").prop('readonly', false); 
					$("#end_date").prop('readonly', false);
					$("#vac_content").prop('readonly', false);
				...
				var resultCallback = function(data){
					fn_EmpDtlWaitingApplyResult(data,loginID,$("#loginIDa").val()); 
				}
	...
	function fn_EmpDtlWaitingApplyResult(data,loginID,loginIDa){
			...
			if($("#userType").val() != "A"){
		   	...
		   	$("#userVacType").val(data.empSelectWaitingApplyModel.vac_type);
			$("#form_content").text('승인대기중에는 수정 또는 삭제 가능');
			$("#currentDate").val(data.empSelectWaitingApplyModel.vac_rdate);
			$("#nowdate").val(new Date().format("yyyy-MM-dd hh:mm:ss")); 
...
<form id="myForm" action=""  method="">
	...
	<input type="hidden" name="userVacType" id="userVacType" value="">
				...
				<table class="row">
					...
					<tbody>
						<tr>
							...
							<td>
								<select id="fvactype" name="fvactype" style="width: 90px"></select>
							</td>
						</tr>
						<tr>
						...
						<tr>
						<td colspan="4">
						<textarea class="inputTxt p100" name="rejectre" id="rejectre" placeholder="반려 사유"></textarea>
						</tr>
						</c:if>
