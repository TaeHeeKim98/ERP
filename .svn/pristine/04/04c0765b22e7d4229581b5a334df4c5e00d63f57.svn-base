...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
							...
							<c:if test="${totalCntEmpTaApply > 0 }">
								...
								<c:forEach items="${listEmpTaApply}" var="list">
									<tr onclick="fn_EmpTaApplyDtlModal(${list.row_num}, '${list.vac_stat}', '${list.loginID}');" style="cursor: pointer;">
										<td>${list.row_num}</td>
										<td>${list.vac_type}</td>
										<td>${list.loginID}</td>
										<td>${list.name}</td>
							...
							<input type="hidden" id="loginIDa" name="loginIDa" value ="${loginID}"/>